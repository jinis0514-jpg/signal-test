...
export default function SignalReasonPanel({ items = [], title = '진입 근거 · 현재 상태', className = '' }) {
  ...
  if (list.length === 0) {
    return (
      <div
        className={cn(
          'rounded-[8px] border border-slate-200 bg-white px-4 py-5 text-[13px] text-slate-500 dark:border-gray-700 dark:bg-gray-900/40 dark:text-slate-400',
          className,
        )}
      >
        진입 근거 없음
  ...
  return (
    <div
      className={cn(
        'rounded-[8px] border border-slate-200 bg-white px-4 py-4 dark:border-gray-700 dark:bg-gray-900/50',
        className,
      )}
    >
